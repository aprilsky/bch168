 ...
 * 如果您的模板要进行修改，请修改 templates/default/modules/user/group_order_address.html
 * 如果您的模型要进行修改，请修改 models/modules/user/group_order_address.php
 *
...
if(filemtime("templates/default/modules/user/group_order_address.html") > filemtime(__file__) || (file_exists("models/modules/user/group_order_address.php") && filemtime("models/modules/user/group_order_address.php") > filemtime(__file__)) ) {
	tpl_engine("default","modules/user/group_order_address.html",1);
	include(__file__);
} else {
/* debug模式运行生成代码 结束 */
?><?php
...
<?php  require("shop/index_header.php");?>
  ...
  <div class="cont">
     ...
     <li class="now"><?php echo $m_langpackage->m_u_first;?>:<br /><?php echo $m_langpackage->m_sure_ordernow;?></li>
     ...
     <li style="padding-right:0"><?php echo $m_langpackage->m_u_third;?>:<br /><?php echo $m_langpackage->m_accomplish;?></li>
		...
		<form action="modules.php?app=user_order_groupbuy&gid=<?php echo  $group_id;?>" method="post" name="for<?php echo  $m_langpackage->m_profile;?>" onsubmit="return checkform();">
		...
		<table width="100%" class="form_table">
			...
			<tr>
				<td class="center"><input type="radio" name="address_id" value="<?php echo $val['address_id'];?>" onclick="changeurl(this.value);" <?php if($val['address_id']==$address_id){?> checked <?php }?> /></td>
		...
		<?php if(!empty($address_rs)){?>
				<table id="oldAddress" width="100%" border="0" cellspacing="0">
					<tr>
						<th align="left" width="200">&nbsp;&nbsp;<?php echo  $m_langpackage->m_order_getsting;?></th><th align="right"><span></span></th>
					</tr>
					...
					<tr>
						...
						<td>
							  <?php echo $areas_info[0][$val['user_country']]['area_name'];?>
                              <?php echo $areas_info[1][$val['user_province']]['area_name'];?>
                              <?php echo $areas_info[2][$val['user_city']]['area_name'];?>
                              <?php echo $areas_info[3][$val['user_district']]['area_name'];?>
						</td>
					</tr>
			...
			<input type="submit" name="submit" value="<?php echo  $m_langpackage->m_post_order;?>" /></td></tr>
				...
				<?php }?>
       <table id="newAddress" width="100%" border="0" cellspacing="0" style="<?php echo  $newaddress;?>">
					<tr>
					...
					<tr><td></td><td><span><?php echo  $m_langpackage->m_sureaddress_rcgoods;?></span><input type=hidden name="newadd" id="newadd" <?php if(empty($address_rs)){?>  value="1" <?php }?> /></td></tr>
